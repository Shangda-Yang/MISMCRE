...
switch type
    case 1
        [samp1] = LGCprior(lx,ly,rate,a,b,mu,1);
...
%         v1 = sqrt(1-beta^2).*varargin{1}(:) + beta.*samp1(:);
        ...
        llik1 = sum(fun1(:)-mu) -  sum(exp(v1(:)))/2^lx/2^ly;
        
        varargout{1} = v1;
        varargout{2} = llik1;
    case 2
        [samp1,samp2] = LGCprior(lx,ly,rate,a,b,mu,2);
        v1 = sqrt(1-beta^2).*(varargin{1}(:) - mu) + mu + beta.*(samp1(:)-mu);
...
%         v2 = sqrt(1-beta^2).*varargin{2}(:) + beta.*samp2(:);
        ...
        llik2 = sum(fun2(:)-mu) -  sum(exp(v2(:)))/2^lx/2^ly*2;
        ...
        varargout{4} = llik2;        
    case 3
        [samp1,samp2] = LGCprior(lx,ly,rate,a,b,mu,3);
        v1 = sqrt(1-beta^2).*(varargin{1}(:) - mu) + mu + beta.*(samp1(:)-mu);
        v2 = sqrt(1-beta^2).*(varargin{2}(:) - mu) + mu + beta.*(samp2(:)-mu);
%         v1 = sqrt(1-beta^2).*varargin{1}(:) + beta.*samp1(:);
%         v2 = sqrt(1-beta^2).*varargin{2}(:) + beta.*samp2(:);
        ...
        varargout{4} = llik2;  
    case 4
        [samp1,samp2,samp3,samp4] = LGCprior(lx,ly,rate,a,b,mu,4);
        v1 = sqrt(1-beta^2).*(varargin{1}(:) - mu) + mu + beta.*(samp1(:)-mu);
        v2 = sqrt(1-beta^2).*(varargin{2}(:) - mu) + mu + beta.*(samp2(:)-mu);
...
%         v4 = sqrt(1-beta^2).*varargin{4}(:) + beta.*samp4(:);
        ...
        llik3 = sum(fun3(:)-mu) -  sum(exp(v3(:)))/2^lx/2^ly*2;
        fun4 = interpob(reshape(v4,size(samp4)),2^lx/2,2^ly/2,xpos,ypos);
        llik4 = sum(fun4(:)-mu) -  sum(exp(v4(:)))/2^lx/2^ly*4;
        ...
        varargout{8} = llik4;          
    case 5
        [samp1,samp2] = LGCprior(lx,ly,rate,a,b,mu,5);
        v1 = sqrt(1-beta^2).*(varargin{1}(:) - mu) + mu + beta.*(samp1(:)-mu);
        v2 = sqrt(1-beta^2).*(varargin{2}(:) - mu) + mu + beta.*(samp2(:)-mu);
%         v1 = sqrt(1-beta^2).*varargin{1}(:) + beta.*samp1(:);
%         v2 = sqrt(1-beta^2).*varargin{2}(:) + beta.*samp2(:);
        ...
        llik2 = sum(fun2(:)-mu) -  sum(exp(v2(:)))/2^lx/2^ly*4;
