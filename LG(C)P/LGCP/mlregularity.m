...
k = 2;
tic;
[we,se,cost] = ml_LGC_emp(@opre_mlLGC_l,N,M,Lmax,data,k,rate,a,b,mu);
toc;

save('./workingdata/ml_LGC_emperical.mat','Lmax','k',...
    'cost','we','se')

figure(1)
set(gcf,'Position',[300 300 1200 500]);
subplot(1,2,2)
plot(2:Lmax,log2(se(2,:)),'--or','Linewidth',2)
hold on
line = refline(-(rate-1),2);
line.LineStyle = '--';
line.Color = 'k';
grid on
xticks(5:Lmax)
...
subplot(1,2,1)
plot(2:Lmax,log2(we(2,:)),'--or','Linewidth',2)
hold on
line = refline(-(rate-1)/2,-2);
line.LineStyle = '--';
line.Color = 'k';
grid on
xticks(5:Lmax)
