% ----------------------------------------------------------------- %
% function [sums, cost] = opre_mlLGC_l(lx,ly,N,data,rate,a,b,mu,k)
...
% ----------------------------------------------------------------- %
function [sums, cost] = opre_mlLGP_l(lx,ly,N,data,rate,a,b,mu,k)
sums(1:3) = 0;
ESSmin = round(N/2);
xpos = data(:,1);
ypos = data(:,2);
...
for i = 1:N
    ...
    if lx == k && ly == k
        ...
        [prior1,prior2] = LGPprior(lx,ly,rate,a,b,mu,5);
        fun1  = interpob(prior1,Nx,Ny,xpos,ypos);
        fun2  = interpob(prior2,Nx/2,Ny/2,xpos,ypos);
        llik1 = sum(fun1(:)-mu) -  length(fun1(:))*log(sum(exp(prior1(:)))/Nx/Ny);
        llik2 = sum(fun2(:)-mu) -  length(fun2(:))*log(sum(exp(prior2(:)))/Nx/Ny*4);
        f1(i) = llik1;
        f2(i) = llik2;
        u1(i,:) = prior1(:);
        u2(i,:) = prior2(:);
