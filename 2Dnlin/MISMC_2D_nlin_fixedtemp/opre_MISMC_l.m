% ------------------------------------------------------------------ %
% [sums, cost] = opre_MISMC_l(lx,ly,N,sigma,data,x_data,K)    
...
parfor i = 1:N
    ...
    if rly <= K
        ...
        [g_2, ~] = pde_solver_2D(rlx,rly-1,x_data,u(i,:));
    ...
    if rlx <= K
        ...
        [g_3, ~] = pde_solver_2D(rlx-1,rly,x_data,u(i,:));
    ...
    if rlx <= K || rly <= K
        ...
        [g_4, ~] = pde_solver_2D(rlx-1,rly-1,x_data,u(i,:));
...
%              rate = 0;
        for i = 1:8
            ...
            if rly <= K
                ...
                [g_2, ~] = pde_solver_2D(rlx,rly-1,x_data,u_star);
                l_2_temp = exp(-0.5/sigma/sigma*(g_2 - y)'*(g_2 - y));
            end
            ...
            if rlx <= K
                ...
                [g_3, ~] = pde_solver_2D(rlx-1,rly,x_data,u_star);
                l_3_temp = exp(-0.5/sigma/sigma*(g_3 - y)'*(g_3 - y));
            end
            ...
            if rlx <= K || rly <= K
                ...
                [g_4, ~] = pde_solver_2D(rlx-1,rly-1,x_data,u_star);
                l_4_temp = exp(-0.5/sigma/sigma*(g_4 - y)'*(g_4 - y));
            end
